 ...
 */
export const getTestNetImage = (networkType: string): string | undefined => {
  if (
    networkType === SEPOLIA ||
...
export const getTestNetImageByChainId = (chainId: string): string | undefined => {
 ...
 */
export const isTestNet = (chainId: string): boolean => TESTNET_CHAIN_IDS.includes(chainId);
...
export function getNetworkTypeById(id: string | number): string {
  ...
  const filteredNetworkTypes = NetworkListKeys.filter(
    (key) => NetworkList[key].networkId === parseInt(id as string, 10),
  );
...
export function getDefaultNetworkByChainId(chainId: string): NetworkInfo | undefined {
  if (!chainId) {
    throw new Error(NetworkSwitchErrorType.missingChainId);
  }

  let returnNetwork: NetworkInfo | undefined;
...
export function findBlockExplorerForNonEvmChainId(chainId: string): string | undefined {
  const blockExplorerUrls =
    MULTICHAIN_NETWORK_BLOCK_EXPLORER_FORMAT_URLS_MAP[chainId as `${string}:${string}`];
...
export function findBlockExplorerForNonEvmAccount(internalAccount: InternalAccount): string | undefined {
  ...
  if (
    ...
    const matchingNetwork = Object.values(networkConfigs || {}).find(
      (network: Record<string, unknown>) => internalAccount.scopes?.includes(network.chainId as string),
    );

    if (matchingNetwork) {
      scope = (matchingNetwork as Record<string, unknown>).chainId as string;
    }
  }
  ...
  const blockExplorerFormatUrls =
    MULTICHAIN_NETWORK_BLOCK_EXPLORER_FORMAT_URLS_MAP[scope as `${string}:${string}`];
 ...
 */
export function getBlockExplorerName(blockExplorerUrl: string): string | undefined {
...
export const getNetworkNameFromProviderConfig = (providerConfig: ProviderConfig): string => {
  ...
  if (providerConfig.nickname) {
    ...
    name = NetworkList?.[networkType as string]?.name || NetworkList[RPC].name;
  }
  return name;
};
 ...
 */
export const getNetworkImageSource = ({ networkType, chainId }: { networkType?: string; chainId: string }): string | undefined => {
...
export const isChainPermissionsFeatureEnabled: boolean = true;

export const isPermissionsSettingsV1Enabled: boolean =
  process.env.MM_PERMISSIONS_SETTINGS_V1_ENABLED === 'true';

export const isPortfolioViewEnabled = (): boolean =>
  process.env.PORTFOLIO_VIEW === 'true';

export const isMultichainV1Enabled = (): boolean => process.env.MULTICHAIN_V1 === 'true';
