...
const getBaseModel = (baseModel: number | undefined): { width: number; height: number } => {
  if (baseModel === 1) {
...
const _getSizes = (scaleVertical: boolean, baseModel: number | undefined): { currSize: number; baseScreenSize: number } => {
...
const scaleVertical = (size: number, options?: Omit<ScaleOptions, 'scaleVertical'>): number =>
