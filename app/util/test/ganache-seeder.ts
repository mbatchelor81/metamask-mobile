...
import type { HstContractFactory, SmartContractName } from './smart-contracts';
...
class GanacheSeeder {
  ...
  ganacheProvider: unknown;

  constructor(ganacheProvider: unknown) {
  ...
  async deploySmartContract(contractName: SmartContractName): Promise<void> {
    const ethersProvider = new Web3Provider(
      this.ganacheProvider as ConstructorParameters<typeof Web3Provider>[0],
      'any',
    );
    const signer = ethersProvider.getSigner();
    const fromAddress = await signer.getAddress();
    const contractFactory = new ContractFactory(
      contractConfiguration[contractName].abi as string[],
    ...
    if (contractName === SMART_CONTRACTS.HST) {
      const hstConfig = contractConfiguration[SMART_CONTRACTS.HST] as HstContractFactory;
