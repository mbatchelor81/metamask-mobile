...
type NumericValue = number | string | BigNumber | typeof BN.prototype;
...
interface AddSubtractOptions extends Partial<ConverterParams> {
  aBase: number;
  bBase: number;
}

interface MultiplyOptions extends Partial<ConverterParams> {
...
const toBigNumber: Record<NumericBase, (n: NumericValue) => BigNumber> = {
  hex: (n: NumericValue): BigNumber => new BigNumber(stripHexPrefix(n as string), 16),
  dec: (n: NumericValue): BigNumber => new BigNumber(String(n), 10),
  BN: (n: NumericValue): BigNumber => new BigNumber(n.toString(16), 16),
};
...
const baseChange: Record<NumericBase, (n: BigNumber) => string | typeof BN.prototype> = {
  hex: (n: BigNumber): string => n.toString(16),
  dec: (n: BigNumber): string => new BigNumber(n).toString(10),
  BN: (n: BigNumber): typeof BN.prototype => new BN(n.toString(16)),
};

// Utility function for checking base types
const isValidBase = (base: unknown): boolean => Number.isInteger(base) && (base as number) > 1;
...
const converter = ({
  ...
  roundDown,
}: ConverterParams): BigNumber | string | typeof BN.prototype => {
...
): ReturnType<typeof converter> | number => {
...
const addCurrencies = (
  a: NumericValue,
  b: NumericValue,
  options: AddSubtractOptions = { aBase: 10, bBase: 10 },
): ReturnType<typeof converter> => {
...
const multiplyCurrencies = (
  a: NumericValue,
  b: NumericValue,
  options: MultiplyOptions = { multiplicandBase: 10, multiplierBase: 10 },
): ReturnType<typeof converter> => {
...
const toNegative = (
  n: NumericValue,
  options: MultiplyOptions = { multiplicandBase: 10, multiplierBase: 10 },
): ReturnType<typeof converter> => multiplyCurrencies(n, -1, options);
